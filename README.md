...
**Two kinds of results are published here and they must not be confused.** The *scripted self-tests* ([report](results/report.md)) use five hand-written fixtures, including policies read straight from the answer key; they validate the evaluator and say nothing about any model. The *measured model run* ([report](results/model_runs/REPORT.md)) is a real evaluation of three Mistral models over all ten cases on 8 September 2026, with prompts, raw responses and request digests committed. Each model was run twice: once with the workload disclosed as exact action/resource pairs, and once with it described in prose so the model had to infer the required permissions. Disclosed, all three scored 8 correct, 0 broken, 2 invalid. In prose, the correct rate fell for all three and broken results appeared for the first time — every one of them the same failure, a policy that can list IAM policy versions but not read one. Both invalid results in every run are GCP role identifiers that do not exist, [verified against Google's published role reference](results/model_runs/gcp_role_verification.md) rather than assumed.
